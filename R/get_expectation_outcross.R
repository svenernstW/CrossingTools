...
#' \itemize{
#'   \item \code{EGEBV}: the mid-parent mean of parental additive genomic values.
#'   \item \code{ETGV}: the predicted mean total genotypic value of the F1, combining additive
...
#'   \code{effects.D}. If \code{marker.mat} is \code{NULL}, it is constructed as
#'   \code{hap.mat1 + hap.mat2}.
#' @param hap.mat1,hap.mat2 Optional numeric haplotype matrices (individuals x markers). Used only
#'   when \code{marker.mat} is \code{NULL}.
#' @param effects.A Numeric matrix of additive marker effects (markers in rows, traits in columns).
#'   Must satisfy \code{nrow(effects.A) == ncol(marker.mat)}.
#' @param effects.D Optional numeric matrix of dominance marker effects (markers in rows, traits in columns).
#'   Must have the same dimensions as \code{effects.A}. If \code{NULL}, dominance is ignored (treated as zero).
#' @param weights Optional numeric vector of trait weights of length \code{ncol(effects.A)}. If provided,
#'   index values \code{EGEBV.IDX} and \code{ETGV.IDX} are computed per cross as weighted sums across traits.
...
#'     \item \code{index.df}: a data.frame with \code{parent1}, \code{parent2}, \code{EGEBV.IDX}, \code{ETGV.IDX}
#'   }
#'
#' @export

#' 
get_expectation_outcross <- function(crosses,  marker.mat, hap.mat1, hap.mat2, effects.A, effects.D=NULL,  weights = NULL,
                                   nthreads = 4L) {
  as.integer(nthreads)

  if (is.null(marker.mat) && (is.null(hap.mat1) || is.null(hap.mat2))) {
    stop("Either marker.mat OR both hap.mat1 and hap.mat2 must be provided.")
  }
  ...
  if (ncol(effects.D) != ncol(effects.A)) { stop("effects.A and effects.D need to have the same dimensions") }
  ...
  name_vec <- paste0(rep(c("EGEBV","ETGV"), each = ncol(effects.A)), seq_len(ncol(effects.A)))
  ...
  if (calculate.index) {
    ...
    names(temp2) <- c("EGEBV.IDX","ETGV.IDX")
