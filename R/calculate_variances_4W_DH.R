...
#' @param calculate.index Logical. If \code{TRUE}, calculate the index based on weights
#'  based on segregation (co)variances. This only works if covariance is TRUE, else will be ignored.
#' @param weights a vector of length equal to the number of traits with values representing the weight
#'
...
calculate_variances_4W_DH <- function(crosses, genetic.map, M, U, t, intensity,
  ...
  # If covariance is disabled (including the 1-trait shortcut), index is not available
