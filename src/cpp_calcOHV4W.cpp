...
// [[Rcpp::export]]
DataFrame cpp_calcOHV4W(const NumericMatrix& Crosses,
                      const List& HBlocks,
  ...
  ct_parallel_for(0, numCrosses, [&](int x) {
    ...
    int P3_index = static_cast<int>(Crosses(x, 2)) - 1;
    int P4_index = static_cast<int>(Crosses(x, 3)) - 1;
    
    // Validate parent indices
    if (P1_index < 0 || P1_index >= nInd || P2_index < 0 || P2_index >= nInd|| P3_index < 0 || P3_index >= nInd|| P4_index < 0 || P4_index >= nInd) {
      // leave OHV_col[x] as 0
      return;
    }
    
    // OHV: sum over blocks of max(LocalGEBV[P1, b], LocalGEBV[P2, b])
    rowvec max_values = arma::max(
      arma::max(LocalGEBV.row(P1_index), LocalGEBV.row(P2_index)),
      arma::max(LocalGEBV.row(P3_index), LocalGEBV.row(P4_index))
    );
